...
| 形象与动态背景 | female / male 两套形象卡、12 个背景变体和角色头像；聊天设置可按「性别」选择，背景按用户时区、睡眠、运动、情绪和关系阶段确定性解析；客户端 Coil 加载，失败时回退渐变 | 默认关闭；公开仓库仅渐变占位图和占位头像，真实成图在私有部署，建议先用 canary |
