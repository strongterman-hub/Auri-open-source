...
当前开源版本基于 Android **0.3.30 / versionCode 33** 及 2026-09-12 的配套服务端。此版按小米、OPPO/一加/realme、vivo、华为、荣耀、三星和魅族提供不同的后台设置引导，修正通知总开关判断、后台连接生命周期和推送标识自恢复，并将 JPush 升级到 6.2.1。它仍使用极光自有通道，不包含需要厂商资质的小米、华为、OPPO 或 vivo 系统推送通道。官方服务、源码和第三方自部署实例是不同的运行环境；开源不包含官方服务的用户数据、访问权限、签名或供应商凭据。

## Auri 能做什么

2026-09-14 服务端新增事件抽取持久重试、相关事件召回、纠错保留、健康数据口径和事实回复校验；主动分享与日程提醒也会检查近期情境。实现边界与配置见[记忆理解与纠错闭环](docs/MEMORY_UNDERSTANDING.md)。Android 版本保持不变。
