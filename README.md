...
2026-09-15 服务端新增可配置人设与关系行为：JSON 预设、关系状态、open loops、聊天篇幅锚定和高频主动品类配额；2026-09-14 新增事件抽取持久重试、相关事件召回、纠错保留、健康数据口径和事实回复校验。实现边界与配置见[配置参考](docs/CONFIGURATION.md)和[版本与验证记录](docs/RELEASE_NOTES.md)。Android 0.3.31 新增智能背景：聊天页按用户时区和状态切换 12 个变体，账号中心可关闭；公开仓库只提供渐变占位图，真实成图仅在私有部署。
...
| 形象与动态背景 | female / male 两套形象卡与 12 个背景变体，按用户时区、睡眠、运动、情绪和关系阶段确定性解析；客户端 Coil 加载，失败时回退渐变 | 默认关闭；公开仓库仅渐变占位图，真实成图在私有部署，建议先用 canary |
