...
当前开源版本基于 Android **0.3.33 / versionCode 36** 及 2026-09-16 的配套服务端。此版按小米、OPPO/一加/realme、vivo、华为、荣耀、三星和魅族提供不同的后台设置引导，修正通知总开关判断、后台连接生命周期和推送标识自恢复，并将 JPush 升级到 6.2.1。它仍使用极光自有通道，不包含需要厂商资质的小米、华为、OPPO 或 vivo 系统推送通道。官方服务、源码和第三方自部署实例是不同的运行环境；开源不包含官方服务的用户数据、访问权限、签名或供应商凭据。

## Auri 能做什么

2026-09-15 服务端新增可配置人设与关系行为：JSON 预设、关系状态、open loops、聊天篇幅锚定和高频主动品类配额；2026-09-14 新增事件抽取持久重试、相关事件召回、纠错保留、健康数据口径和事实回复校验。实现边界与配置见[配置参考](docs/CONFIGURATION.md)和[版本与验证记录](docs/RELEASE_NOTES.md)。Android 0.3.31 新增智能背景：聊天页按用户时区和状态切换背景变体，账号中心可关闭；0.3.32 开放 Auri 人设用户选择，0.3.33 进一步让聊天背景中下部更清晰、助手气泡在智能背景下透出角色，并让聊天设置头像跟随所选性别。服务端会读取最新天气观测：下雨切到撑伞场景、下雪切到雪景、夏季/高温切到夏装、秋季/低温切到秋装叠穿，不需要更新 Android 客户端。公开仓库只提供渐变占位图和占位头像，真实成图与头像仅在私有部署。
...
| 人设与关系 | 可配置人设预设、关系状态、open loops 和行为边界；聊天按输入长度锚定篇幅，低信息消息可沉默/延后，未结问题阻止重复提问；聊天设置可选择「性格」 | 预设 JSON 可部署覆盖；用户选择需 `AURI_PERSONA_USER_SELECTION_ENABLED=true` |
| 形象与动态背景 | female / male 两套形象卡、16 个背景变体和角色头像；聊天设置可按「性别」选择，背景按用户时区、睡眠、运动、情绪、关系阶段和最新天气/季节确定性解析（雨天撑伞、雪景、夏装、秋装叠穿）；客户端 Coil 加载，失败时回退渐变 | 默认关闭；公开仓库仅渐变占位图和占位头像，真实成图在私有部署，建议先用 canary |
...
- [人设形象与动态背景](docs/PORTRAIT_BACKGROUND.md)
