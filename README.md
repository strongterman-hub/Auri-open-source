...
| 主动联系 | 根据近期问答、记忆、健康、位置、天气等证据决定是否联系；问题发送前检查是否已经回答 | 全局配置和用户开关控制；无关或不新鲜证据不能充当当前事实 |
...
Auri 不是每隔固定时间发一句随机问候。定时任务或健康/天气等事件先触发评估，再经过免打扰、节奏和待回复数量检查。模型需要给出是否联系的判断与证据引用，投放前还会核验。近期用户回答与它所回复的问题独立保留；提问、记忆回访和探索草稿还会在发送前检查是否已知答案，必要时延续改写或静默，而不是从零重问。
