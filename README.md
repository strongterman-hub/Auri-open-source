...
当前开源版本基于 Android **0.3.30 / versionCode 33** 及 2026-09-15 的配套服务端。此版按小米、OPPO/一加/realme、vivo、华为、荣耀、三星和魅族提供不同的后台设置引导，修正通知总开关判断、后台连接生命周期和推送标识自恢复，并将 JPush 升级到 6.2.1。它仍使用极光自有通道，不包含需要厂商资质的小米、华为、OPPO 或 vivo 系统推送通道。官方服务、源码和第三方自部署实例是不同的运行环境；开源不包含官方服务的用户数据、访问权限、签名或供应商凭据。

## Auri 能做什么

2026-09-15 服务端新增可配置人设与关系行为：JSON 预设、关系状态、open loops、聊天篇幅锚定和高频主动品类配额；2026-09-14 新增事件抽取持久重试、相关事件召回、纠错保留、健康数据口径和事实回复校验。实现边界与配置见[配置参考](docs/CONFIGURATION.md)和[版本与验证记录](docs/RELEASE_NOTES.md)。Android 版本保持不变。

| 能力 | 已有实现 | 使用条件与边界 |
|---|---|---|
| 连续聊天 | 同一账号持续会话、分页历史、客户端缓存、消息幂等重试 | 默认不按天重置 |
| 好友式回应 | 先确认发送，后台分别规划回复时机与 `micro/short/normal/detailed` 篇幅 | 问题和任务不能因自然收尾或短回复策略而丢失必要内容 |
| 长期记忆 | 用户偏好、持久笔记、结构化事件、纠错与渐进遗忘 | 记忆质量取决于模型；需要持续观察 |
| 主动联系 | 根据近期问答、记忆、健康、位置、天气等证据决定是否联系；问题发送前检查是否已经回答 | 全局配置和用户开关控制；个人睡眠画像与新鲜分期会延后日常打扰，并保留有界自动解锁 |
| 主动节奏 | 区分分享与提问、曝光确认、回复归因、冷却和恢复探测 | 未曝光不能直接视为用户拒绝互动 |
| 人设与关系 | 可配置人设预设、关系状态、open loops 和行为边界；聊天按输入长度锚定篇幅，低信息消息可沉默/延后，未结问题阻止重复提问 | 预设 JSON 可部署覆盖；用户选择 API 已预留，默认关闭 |
...
Auri 不是每隔固定时间发一句随机问候。定时任务或健康/天气等事件先触发评估，再经过免打扰、个人睡眠、节奏和待回复数量检查。个人睡眠窗口从近期主睡眠形成画像；滚动同步的临时结束点只有在接近预计醒来时间且输入稳定后才会提前放行，数据缺失时仍会在受最大窗口约束的自适应截止自动解锁。模型随后给出是否联系的判断与证据引用，投放前还会核验。近期用户回答与它所回复的问题独立保留；提问、记忆回访和探索草稿还会在发送前检查是否已知答案，必要时延续改写或静默，而不是从零重问。

投放账本分别记录聊天落库、推送接受、客户端曝光、用户回复和结算。纯分享不要求回复，未知曝光不直接累计未回复惩罚。用户长时间不回复后会进入休息并逐步尝试低压恢复；用户也可以直接在聊天里表达“少找我一点”或“先安静”。如果用户明确指出“刚说过”或“怎么又问”，该反馈不会被算作积极回应，并会暂时停止新的互动式追问。
...
- **省略式回答**：事件抽取会读取当前消息前的有限对话来理解“弄好了”“没差别”等回答所指，但只有当前用户消息可成为新增 owner 事实。
