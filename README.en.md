...
Android 0.3.33 (versionCode 36) adds persona selection and visual polish: chat settings expose presentation ("Gender") and personality ("Personality"), the settings avatar follows the selected presentation, and smart backgrounds keep more of the character visible through lighter scrims and translucent assistant bubbles. The server now also matches the latest weather observation: rain selects an umbrella scene, snow selects a snowy scene, hot summer weather selects light clothing, and autumn/cold weather selects layered clothing. Existing app builds pick these variants up through the normal background refresh, so no Android update is required. This builds on the 0.3.30 release, which added brand-specific background-settings guidance for Xiaomi, OPPO/OnePlus/realme, vivo, Huawei, Honor, Samsung, and Meizu and improved notification checks, background connections, JPush registration, and JPush 6.2.1. This remains JPush's own channel; vendor system channels and their credentials are not included. A complete offline privacy policy is available in the app, with the official service policy at [the privacy page](https://auri.thinktocode.online/privacy). Self-hosted deployments must adapt the policy to their own operator and providers.
...
- **Persona and smart background:** female/male character cards, selectable presentation and personality in chat settings, time-/state-/weather-aware chat backgrounds with a character avatar. Rain, snow, summer and autumn variants are resolved from the latest weather observation. Public builds ship gradient placeholders instead of the real artwork.
