...
| `server/app/persona/` | 可配置人设预设、关系状态、形象卡、动态背景解析和逐轮行为策略 |\n| `server/app/static/portrait/` | 12 变体 x 2 形象的背景图；公开仓库为渐变占位图，私有部署可替换为真实成图 |
...
个人睡眠门控由近期规范化主睡眠形成慢变画像，并为每个醒来日固定 episode。新鲜分期和明确用户活动优先；滚动睡眠评分的 `session_end` 只有在不显著早于预计醒来时间且输入已稳定时才作为确认结束。若健康终态长期缺失，预计醒来中心、醒来离散度和最大窗口共同形成有上限的硬截止，确保无需用户先发消息也能自动解锁。睡眠期间只合并触发来源，不重放旧文案；放行原因写入不含正文的门控审计。
