...
4. 回复规划器根据语义和安全/任务规则选择时间档位。问题、附件和明确任务不应静默结束；规划失败回退快速回应。
...
持久笔记采用预算约束；观察记录保存来源；事件提取只使用带时间信息的真实输入。事件包含状态、故事线、来源和关系。纠错通过替代/修正关系处理，避免同一事实同时存在两个冲突答案。
...
decision ledger 保存消息投放、推送结果、客户端 inbox ack、回复和结算；不复制消息正文。互动期待区分 `share`、`soft_check_in`、`direct_question`。只有已曝光且期待回复的消息进入未回复加权计算。冷却、休息和探测恢复替代永久封锁。
