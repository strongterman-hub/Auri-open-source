...
0.3.30 使用 JPush 6.2.1。配置推送的 direct 包会创建 `auri_messages` 消息渠道，并在已登录、同意隐私且系统允许通知时保持后台连接；这与用户是否开启“主动消息”无关，因为异步聊天回复和日程提醒也需要通知。registration ID 变化时会重新登记，同一 ID 每 24 小时允许幂等补登记一次。
